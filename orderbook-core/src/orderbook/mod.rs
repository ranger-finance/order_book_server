...
pub use types::{Coin, InnerOrder, Oid, Px, Side, Sz};

#[derive(Clone, Default)]
pub struct OrderBook<O> {
    oid_to_side_px: HashMap<Oid, (Side, Px)>,
    pub(crate) bids: BTreeMap<Px, LinkedList<Oid, O>>,
    pub(crate) asks: BTreeMap<Px, LinkedList<Oid, O>>,
}

#[derive(Debug, Clone)]
pub struct Snapshot<O>(pub(crate) [Vec<O>; 2]);
...
impl<O: InnerOrder> OrderBook<O> {
    ...
    pub fn add_order(&mut self, mut order: O) {
    ...
    pub fn modify_sz(&mut self, oid: Oid, sz: Sz) -> bool {
    ...
    pub fn to_snapshot(&self) -> Snapshot<O> {
    ...
    #[must_use]
    pub fn from_snapshot(mut snapshot: Snapshot<O>, ignore_triggers: bool) -> Self {
...
#[cfg(test)]
mod tests {
    use crate::orderbook::types::{Coin, Sz};
