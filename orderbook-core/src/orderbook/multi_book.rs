use crate::{
    orderbook::{Coin, InnerOrder, Oid, OrderBook, Snapshot, Sz},
    prelude::*,
};
...
#[derive(Clone)]
pub struct OrderBooks<O> {
    order_books: BTreeMap<Coin, OrderBook<O>>,
}

impl<O: InnerOrder> OrderBooks<O> {
    pub const fn as_ref(&self) -> &BTreeMap<Coin, OrderBook<O>> {
        &self.order_books
    }
    #[must_use]
    pub fn new() -> Self {
    ...
    pub fn add_order(&mut self, order: O) {
        let coin = &order.coin();
        self.order_books.entry(coin.clone()).or_insert_with(OrderBook::new).add_order(order);
    }

    pub fn cancel_order(&mut self, oid: Oid, coin: Coin) -> bool {
    ...
    pub fn modify_sz(&mut self, oid: Oid, coin: Coin, sz: Sz) -> bool {
...
impl<O: Send + Sync + InnerOrder> OrderBooks<O> {
    #[must_use]
    pub fn to_snapshots_par(&self) -> Snapshots<O> {
        let snapshots = self.order_books.par_iter().map(|(c, book)| (c.clone(), book.to_snapshot())).collect();
        Snapshots(snapshots)
    }
}

pub fn load_snapshots_from_str<O, R>(str: &str) -> Result<(u64, Snapshots<O>)>
where
...
pub async fn load_snapshots_from_json<O, R>(path: &Path) -> Result<(u64, Snapshots<O>)>
where
    O: TryFrom<R, Error = Error>,
    R: Serialize + for<'a> Deserialize<'a>,
{
