...
impl<K: Clone + Eq + Hash, T: Clone> LinkedList<K, T> {
    ...
    pub fn push_back(&mut self, key: K, value: T) -> bool {
    ...
    pub fn head_value_ref_mut_unsafe(&mut self) -> Option<&mut T> {
    ...
    pub fn node_value_mut(&mut self, key: &K) -> Option<&mut T> {
        if let Some(sid) = self.key_to_sid.get(key) {
            Some(&mut self.slab[*sid].value)
        } else {
            None
        }
    }
    ...
    pub fn fold<F, Acc>(&self, mut init: Acc, f: F) -> Acc
    where
        F: Fn(&mut Acc, &T),
    {
        let mut cur = self.head;
...
impl<K: Clone + Eq + Hash, T: Clone> Default for LinkedList<K, T> {
    fn default() -> Self {
        Self::new()
    }
}
