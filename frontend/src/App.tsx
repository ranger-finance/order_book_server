...
function App() {
  ...
  const { orderbooks, isConnected, error } = useWebSocket(selectedExchange, selectedToken);
  ...
  return (
    <div className="app">
      <header className="header">
        ...
        <div className="controls">
          <select
            ...
            onChange={(e) => setSelectedExchange(e.target.value as Exchange | "all")}
      ...
      {error && <div className="error-banner">{error} - Attempting to reconnect...</div>}

      {orderbookList.length === 0 ? (
        <div className="loading">
          <p>Waiting for orderbook data...</p>
          <p className="hint">Make sure the backend server is running on localhost:8080</p>
        </div>
      ) : (
        <div className="dashboard">
          {orderbookList.map((update) => {
            if (selectedExchange !== "all" && update.orderbook.exchange !== selectedExchange) {
              return null;
            }
            if (selectedToken !== "all" && update.orderbook.symbol !== `${selectedToken}-PERP`) {
              return null;
            }
